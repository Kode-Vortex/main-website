...
export const LampContainer = ({
  ...
  return (
    <div
      className={cn(
        "relative flex min-h-[40rem] pt-24 bg-red-400 w-full flex-col items-center justify-center overflow-hidden z-0",
        className
      )}
    >
      <div className="relative flex w-full flex-1 scale-y-150  items-center justify-center isolate z-0">
        ...
        <div className="absolute inset-auto z-50 h-44 w-[60rem] -translate-y-1/2 rounded-full bg-cyan-500 opacity-50 blur-3xl"></div>
        <motion.div
          initial={{ width: "8rem" }}
          ...
          className="absolute inset-auto z-30 h-16 w-96 -translate-y-[6rem] rounded-full bg-cyan-400 blur-2xl"
        ></motion.div>
        <motion.div
          initial={{ width: "15rem" }}
      ...
      <div className="relative z-50 bottom-80 flex flex-col items-center px-5 translate-y-20">
        {children}
      </div>
    </div>
  );
